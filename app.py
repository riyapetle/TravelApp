...
def create_pdf(itinerary_text, source, destination, country, images):
    ...
    html_content = markdown.markdown(itinerary_text, extensions=['tables'])
    
    # Generate image grid HTML
    images_html = "<h2>Trip Gallery</h2><div class='gallery'>"
    for img, label in images:
        if img:
            b64 = pil_to_b64(img)
            images_html += f"""
            <div class='img-card'>
                <img src='data:image/png;base64,{b64}' width='300px' />
                <p><b>{label}</b></p>
            </div>
            """
    images_html += "</div>"
    
    # Wrap in beautiful styled HTML
    full_html = f"""
    <html>
    <head>
        <meta charset="utf-8">
        <style>
            ...
            h2 {{ color: #2980b9; font-size: 18px; margin-top: 20px; }}
            h3 {{ color: #16a085; font-size: 14px; margin-top: 15px; }}
            p, li {{ font-size: 12px; color: #444; }}
            ...
            .img-card p {{ margin-top: 5px; font-size: 11px; }}
        </style>
    </head>
    <body>
        <h1>AI Student Travel Itinerary</h1>
        <div class="header-info">
            <b>Route:</b> {source} &rarr; {destination}, {country}
        </div>
        
        {images_html}
        <hr style="color: #ecf0f1; margin: 20px 0;">
        {html_content}
        
        <div id="footerContent" style="text-align: center; font-size: 10px; color: #7f8c8d;">
            Generated by AI Student Travel Planner | Page <pdf:pagenumber>
        </div>
    </body>
    </html>
    """
